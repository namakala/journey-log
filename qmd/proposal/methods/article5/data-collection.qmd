...
A large-scale survey will be conducted online following the convenience sampling procedure. As a part of non-probability sampling, convenience sampling is favored due to its simplicity and applicability. Although using a probability sampling method is naturally desirable, the absence of an institutionally accessible internet panel for research purposes in Indonesia renders probability sampling in an online survey impractical. For a practical purpose, we can establish a pseudo-panel comprised of multiple groups of heterogeneous sub-populations by adopting convenience sampling. Although convenience sampling does not directly support generalizability to the general population, it is possible to match respondents' demography data to the basic health research (*riset kesehatan dasar*/Riskesdas), which will allow generalizing specific sub-populations to a certain degree. A list of pseudo-panelists is selected through clustered random sampling by adjusting representativeness using demography data. The disadvantage of this approach is the inability to pre-allocate respondents into clusters, which is only possible by conducting an interim analysis during data collection.
